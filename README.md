...
3. **基于置信度的级联主动研判 (Engine <-> Jev <-> Agent Multimodal)**：
   - **Zero-API 模式（无 Jev Key）**：触发 `[AGENT_ACTION_REQUIRED]` 原生握手协议，引擎导出自解释工单与实拍照片，由宿主智能体运用自身认知推理与视觉能力自主完成研判，真正零外部付费 API 依赖；
   - **Jev 级联模式（配置 Jev Key）**：确定性高的商家在毫秒级快筛决断；模糊区间的复合商家拉取实拍图由宿主智能体调用 `view_file` 肉眼查验招牌与专业设施，闭环回写缓存。
